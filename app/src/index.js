...
function initializeGame() {
    ...
    startBtn.addEventListener("click", () => {
        ...
        document.addEventListener("dragstart", function (event) {
            ...
            if ((dragged == battleship) || (dragged == carrier) || (dragged == destroyer) || (dragged == patrol) || (dragged == submarine)) {
        ...
        document.addEventListener("drop", function (event) {
            event.preventDefault();
            if ((event.target.classList.value.includes("dropzone")) && ((dragged == battleship) || (dragged == carrier) || (dragged == destroyer) || (dragged == patrol) || (dragged == submarine))) {
        ...
        readyBtn.addEventListener("click", () => {
            const playerShips = [Player1.Battleship, Player1.Carrier, Player1.Destroyer, Player1.Patrol, Player1.Submarine];
            if (addPlayerPositions(playerShips, Player1.Gameboard.board) === true) {
                fillGameboard(Player1.Battleship, Player1.Carrier, Player1.Destroyer, Player1.Patrol, Player1.Submarine, Player1.Gameboard.board);
                let computerShips = [Computer.Battleship, Computer.Carrier, Computer.Destroyer, Computer.Patrol, Computer.Submarine];
                ...
                let concatCPUships = computerShips[0].position.concat(computerShips[1].position, computerShips[2].position, computerShips[3].position, computerShips[4].position);
                Player1.Gameboard.generateBoard();
                    ...
                    tile.addEventListener("click", () => {
                        if (tile.classList.contains("game")) {
                            ...
                            for (let i = 0; i < concatCPUships.length; i++) {
                                if (tile.innerText === "x" || tile.innerText === "o" || tile.innerText === "X") {
                                    errorMsg.innerHTML = `
                                ...
                                else if ((tile.id === concatCPUships[i]) && (tile.innerText === "")) {
                                    tile.innerText = "x";
                ...
                function initiateGame() {
                    if (Computer.turn === true) {
                        ...
                        if (/(- Hit)$/.test(e) === true) {
                            computerMakeMove(/\d$/.test(cBoard[cBoard.indexOf(e) + 1]), /\d$/.test(cBoard[cBoard.indexOf(e) - 1]), /\d$/.test(cBoard[cBoard.indexOf(e) + 10]), /\d$/.test(cBoard[cBoard.indexOf(e) - 10]), playerShips, e, cBoard, filtered);
                        }
                        else {
                            let notHit = cBoard.filter((item) => {
                                if ((/(- Hit)$/.test(item) === false) && (/(Miss)$/.test(item) === false) && (/(Sunk)$/.test(item) === false)) {
                                    return item;
                                }
                            });
    ...
    function addPlayerPositions(ships, board) {
        ...
        let countPos = ships[0].position.concat(ships[1].position, ships[2].position, ships[3].position, ships[4].position);
        let valToRet = true;
        ...
        if ((countPos.length < 17) || (countPos.includes(undefined) === true) || (countPos.includes(null) === true)) {
            return false;
        }
        else {
            return valToRet;
    ...
    function checkForOverlap(arr) {
        const combinedArr = arr[0].position.concat(arr[1].position, arr[2].position, arr[3].position, arr[4].position);
    ...
    function assignCPUPos(arr, currentDir) {
        ...
        for (let j = 0; j < arr.length; j++) {
            ...
            else if ((/\d$/.test(arr[j].position[0]) === false) && (/(- Hit)$/.test(arr[j].position[0]) === false) && (/(Sunk)$/.test(arr[j].position[0])) === false) {
                arr[j].position[0] = board[Math.floor(Math.random() * board.length)];
            }
            for (let i = 1; i < arr[j].size; i++) {
                if ((/\d$/.test(arr[j].position[i - 1] === false) && (/(- Hit)$/.test(arr[j].position[i - 1] === false)) && (i >= 2))) {
                    arr[j].splice(1);
                    i = 1;
                }
                else if (arr[j].orientation === "Horizontal") {
                    if ((direction === 0) && (/\d$/.test(board[(board.indexOf(arr[j].position[i - 1])) + 1]) === false) || ((/^J/i.test(board[board.indexOf(arr[j].position[i - 1])]) === true) && (/^A/i.test(board[board.indexOf(arr[j].position[i - 1]) + 1]) === true))) {
                        direction = 1;
                        handleDirectionChange(direction, 1, arr);
                    }
                    else if ((direction === 0) && (/\d$/.test(board[board.indexOf(arr[j].position[i - 1]) + 1]) === true)) {
                        arr[j].position[i] = board[board.indexOf(arr[j].position[i - 1]) + 1];
                    }
                    else if ((direction === 1) && (/\d$/.test(board[board.indexOf(arr[j].position[i - 1]) - 1]) === false) || ((/^A/i.test(board[board.indexOf(arr[j].position[i - 1])]) === true) && (/^J/i.test(board[board.indexOf(arr[j].position[i - 1]) - 1]) === true))) {
                        direction = 0;
                        handleDirectionChange(direction, 1, arr);
                    }
                    else if ((direction === 1) && (/\d$/i.test(board[board.indexOf(arr[j].position[i - 1]) - 1]) === true)) {
                        arr[j].position[i] = board[board.indexOf(arr[j].position[i - 1]) - 1];
                    }
                }
                else if (arr[j].orientation === "Vertical") {
                    if ((direction === 0) && (/\d$/.test(board[board.indexOf(arr[j].position[i - 1]) + 10]) === false)) {
                        direction = 1;
                        handleDirectionChange(direction, 10, arr);
                    }
                    else if ((direction === 0) && (/\d$/.test(board[board.indexOf(arr[j].position[i - 1]) + 10]) === true)) {
                        arr[j].position[i] = board[board.indexOf(arr[j].position[i - 1]) + 10];
                    }
                    else if ((direction === 1) && (/\d$/.test(board[board.indexOf(arr[j].position[i - 1]) - 10]) === false)) {
                        direction = 0;
                        handleDirectionChange(direction, 10, arr);
                    }
                    else if ((direction === 1) && (/\d$/.test(board[board.indexOf(arr[j].position[i - 1]) - 10]) === true)) {
                        arr[j].position[i] = board[board.indexOf(arr[j].position[i - 1]) - 10];
                    }
                }

                function handleDirectionChange(dir, num, arr) {
                    if ((dir === 0) && (/\d$/.test(board[board.indexOf(arr[j].position[i - 1]) + num] === false))) {
                        arr[j].position.splice(0);
                        assignCPUPos(arr, dir);
                    }
                    else if ((dir === 0) && (board[board.indexOf(arr[j].position[i - 1])] !== board[arr[j].position[0]])) {
                        arr[j].position.splice(0);
                        assignCPUPos(arr, dir);
                    }
                    else if ((dir === 0) && (/\d$/.test(board[board.indexOf(arr[j].position[i - 1]) + num]) === true)) {
                        arr[j].position[i] = board[board.indexOf(arr[j].position[i - 1]) + num];
                    }
                    else if ((dir === 1) && (/\d$/.test(board[board.indexOf(arr[j].position[i - 1]) - num]) === false)) {
                        arr[j].position.splice(0);
                        assignCPUPos(arr, dir);
                    }
                    else if ((dir === 1) && (board[board.indexOf(arr[j].position[i - 1])] !== board[arr[j].position[0]])) {
                        arr[j].position.splice(0);
                        assignCPUPos(arr, dir);
                    }
                    else if ((dir === 1) && (/\d$/.test(board[board.indexOf(arr[j].position[i - 1]) - num]) === true)) {
    ...
    function determineFirstTurn() {
        ...
        if (turn === 0) {
            ...
            toast.innerText = "Your turn - click a position on the empty board to attack";
        }
        else {
            Computer.turn = true;
        }
    ...
    function computerMakeMove(a, b, c, d, arr, e, cBoard, filt) {
        ...
        if (/^A/.test(cBoard[cBoard.indexOf(e)]) === true && /^J/.test(cBoard[cBoard.indexOf(e) - 1]) === true) {
            b = false;
        }
        if (/^J/.test(cBoard[cBoard.indexOf(e)]) === true && /^A/.test(cBoard[cBoard.indexOf(e) + 1]) === true) {
            a = false;
        }
        if (cBoard.includes(e)) {
            if (a && b && c && d) {
                ...
                computerMakeMove(/\d$/.test(cBoard[cBoard.indexOf(y) + 1]), /\d$/.test(cBoard[cBoard.indexOf(y) - 1]), /\d$/.test(cBoard[cBoard.indexOf(y) + 10]), /\d$/.test(cBoard[cBoard.indexOf(y) - 10]), arr, y, cBoard, filt);
            }
            else {
                let y = cBoard.filter((pos) => {
                    if ((/(- Hit)$/.test(pos) === false) && (pos !== "Miss") && (/(- Sunk)$/.test(pos) === false)) {
                        return pos;
                    }
                });
    ...
    function checkSunk(arr, mv) {
        ...
        for (let i = 0; i < arr.length; i++) {
            ...
            if (arr[i].position[arr[i].position.length - 1] === "Sunk") {
                ...
                if (arr[i].position[x.length - 1] === arr[i].position[arr[i].position.length - 1]) {
                    p = i;
                    break;
                };
            }
        ...
        if (/\d/.test(p) === true) {
            for (let j = 0; j < arr[p].position.length; j++) {
                ...
                if ((/(- Hit)$/.test(arr[p].position[j]) === true) && (h === arr[p].position.length)) {
                    sunk = true;
                    break;
                }
            }
    ...
    function hitOrMiss(mv, shipArr, board) {
        ...
        if (hitStatus === true) {
            ...
            if (board === Player1.Gameboard.board && sunkStatus.sunk === false) {
            ...
            else if (board === Computer.Gameboard.board && sunkStatus.sunk === false) {
                board.splice(board.indexOf(mv), 1, mv + " - Hit");
                ...
                for (let n = 0; n < shipArr[sunkStatus.index].position.length; n++) {
                    if (shipArr[sunkStatus.index].position[n].match(/^(\w10)/i) !== null) {
                        board.splice(board.indexOf(shipArr[sunkStatus.index].position[n]), 1, shipArr[sunkStatus.index].position[n].match(/^(\w10)/i)[0] + " - Sunk");
                    }
                    else {
                        board.splice(board.indexOf(shipArr[sunkStatus.index].position[n]), 1, shipArr[sunkStatus.index].position[n].match(/^(\w\d)/i)[0] + " - Sunk");
                    }
                ...
                function colorSunkPos(cp) {
                    ...
                    for (let i = 10; i < cp.children.length; i++) {
                        for (let j = 0; j < colorSunk.length; j++) {
                            ...
                            else if (colorSunk[j].match(/^(\w\d -)/i)[0].match(/^(\w\d)/i)[0] === cp.children[i].id) {
                                cp.children[i].innerText = "X";
                                cp.children[i].style.color = "black";
                                cp.children[i].style.backgroundColor = "orangered";
                            }
